...
The web server's settings can be modified in [`webserver.conf`](./configuration.markdown#webserverconf). You might need to edit this file if you're [using an external CA][external_ca] or running Puppet on a non-standard port.
...
Signing and revoking certificates over the network is disallowed by default; you can use the [`ca.conf`](./configuration.markdown#caconf) file to let specific certificate owners issue commands.
...
You can change the port in `webserver.conf` if necessary. See the [Configuration](./configuration.markdown#webserverconf) page for details.
...
* `webserver.conf`: Web server configuration settings.
