...
By default, reading this data from headers is disabled. To allow Puppet Server to recognize it, open `config.d/master.conf` and add  `allow-header-cert-info: true` to the `webserver` config block. See [Puppet Server Configuration](./configuration.markdown) for more information on the `master.conf` file.
...
Mandatory. Must be one of `NONE`, `SUCCESS`, `GENEROUS` or `FAILED:reason`. Puppet Server uses this to authorize requests.
