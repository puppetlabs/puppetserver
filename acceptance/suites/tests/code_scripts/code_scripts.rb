...
teardown do
  puts 'Teardown: Implemented Later.' 
end

step "Clean up garbage from last run." do
...
exit 77
CCC
  create_remote_file(master, '/opt/puppetlabs/server/apps/puppetserver/code_content_script.sh', code_content_command_script)
  on(master, "chown puppet:root /opt/puppetlabs/server/apps/puppetserver/code_content_script.sh")
  on(master, "chmod +x /opt/puppetlabs/server/apps/puppetserver/code_content_script.sh")

  
  step 'SETUP: Configure the code-id and code-command scripts'
...
hocon_setting { 'code-content-command-script' :
  ...
  value => '/opt/puppetlabs/server/apps/puppetserver/code-content_script.sh',
  }
  CICS
...
##########
# Workaround: Disable legacy auth.conf and insert rule for static_file_endpoint
# Workaround: insert tk auth.conf rule that allows access to static_file_endpoint
#  
...
################################################################################
#step "Create a file resource, commit the change"
#step ""
#step "Get the current code-id"
#step "Pull the catalog, validate that it contains the current code-id"
#step "Perform a puppet agent run, validate that the file resource is present."
